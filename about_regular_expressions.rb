...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    assert_equal "a", "abbcccddddeeeee"[/az*/]
    assert_equal "", "abb zcccdd  ddeeeee"[/z*/]
    # THINK ABOUT IT:
    #
    # When would * fail to match?
    #
    assert_equal "", "abb zcccdd  ddeeeee"[/4*/]
    # Никогда? Какой бы один символ не написали, всегда минимум будет ""
    # 
  end
  ...
  # Допустим у нас есть <p> Hello World </p>
  # Можно подумать, что <. +>, (. Означает что-нибудь и + означает повтор)
  # будет только соответствовать "<p>" и "</ p>",
  # когда на самом деле это будет "<p>Hello World</p>"
    def test_greedy_operators
      # К вопросу про жадность пример:
      assert_equal "<p>Hello World</p>", "<p>Hello World</p>"[/<.+>/]
    end
  # ------------------------------------------------------------------
  ...
  def test_variables_can_also_be_used_to_access_captures
    ...
    assert_equal "Gray", $1    
    assert_equal "James", $2     # тут интересно!
  end
  ...
  # [...] - применяется за один шаг
  # (|) - запоминается текущую позициюб для отката во время поочередной проверки каждого элемента
